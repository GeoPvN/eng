...
switch ($action) {
	...
	case 'get_list':
		...
		$rResult = mysql_query("	SELECT 	`users`.`id`,
                                    ...
                                    LEFT JOIN branch ON user_info.branch_id = branch.id
                                    LEFT JOIN service_center ON user_info.service_center_id = service_center.id
...
function AddWorker($user_id, $name, $tin, $position, $address, $image, $password, $home_number, $mobile_number, $comment,  $user, $userpassword, $group_permission, $dep_id, $service_center_id, $branch_id)
{
	...
	mysql_query("INSERT INTO `user_info`
					(`user_id`, `name`, `tin`, `position_id`, `address`, `image`, `home_phone`, `mobile_phone`, `comment`, `dep_id`, `service_center_id`, `branch_id`)
				 VALUES
					($persons_id, '$name', '$tin', $position, '$address', '$image', '$home_number', '$mobile_number', '$comment', '$dep_id', '$service_center_id', '$branch_id')");
