...
switch ($done){
	...
	default:
		$result = mysql_query(" SELECT IF(ISNULL(source_info.`name`),'არ არის შევსებული',source_info.`name`) AS `si_name`,
                                        ...
                                        LEFT JOIN source_info ON personal_info.source_info_id = source_info.id
                                        WHERE DATE(`incomming_call`.`date`) >= '$start' and  DATE(`incomming_call`.`date`) <= '$end' AND personal_info.source_info_id in($method)
                                        )  * 100),2) AS `procent`
                                FROM `incomming_call`
                                JOIN personal_info ON personal_info.incomming_call_id = incomming_call.id
                                LEFT JOIN source_info ON personal_info.source_info_id = source_info.id
		                        WHERE DATE(`incomming_call`.`date`) >= '$start' and  DATE(`incomming_call`.`date`) <= '$end' AND personal_info.source_info_id in($method)
                                GROUP BY source_info.id");
