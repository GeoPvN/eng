...
function GetRecordingsSection1($res)
{
    ...
    $req = mysql_query("SELECT  TIME(`calldate`) AS 'time',
        ...
        FROM     `cdr`
        WHERE     (LENGTH(`src`) = 3 && (`dst` LIKE '%$phone%' || `dst` LIKE '%$phone1%')");
...
function GetRecordingsSection($res)
{
	...
	$req = mysql_query("SELECT   `calldate` AS 'time',
            			         SUBSTR(`userfield`, 7) as userfield
            			FROM     `cdr`
            			WHERE     ( `userfield` != '' && `src` LIKE '%$phone%' AND LENGTH(dst) =3 AND billsec > 2)");
	
	$req1 = mysql_query("SELECT  `calldate` AS 'time',
                	             SUBSTR(`userfield`, 7) as userfield
                	    FROM     `cdr`
                	    WHERE  (`dst` LIKE '%$phone%' && `userfield` != '');");
        ...
        <fieldset style="margin-top: ; width: 333px; float: left;">
            ...
            <table style="width: 100%; border: solid 1px #85b1de; margin:auto;">
                <tr style="border-bottom: solid 1px #85b1de; height: 20px;">
                    <th style="padding-left: 10px;">დრო</th>
                    <th  style="border: solid 1px #85b1de; padding-left: 10px;">ჩანაწერი</th>
	                <th  style="border: solid 1px #85b1de; padding-left: 10px;">შემომავალი გამავალი</th>
                </tr>';
	if (mysql_num_rows($req) == 0){
		$data .= '<td colspan="2" style="height: 20px; text-align: center;">ჩანაწერები ვერ მოიძებნა</td>';
	}
	...
	while( $res2 = mysql_fetch_assoc($req)){
		...
		$data .= '
                <tr style="border-bottom: solid 1px #85b1de; height: 20px;">
                    ...
                    <td style="vertical-align: middle; text-align: center;"><button class="download" str="' . $src . '">მოსმენა</button></td>
                    <td style="vertical-align: middle; text-align: center;">შემომავალი</td>
                </tr>';
	}
	
	while( $res3 = mysql_fetch_assoc($req1)){
	    $src1 = $res3['userfield'];
	    $data .= '
                <tr style="border-bottom: solid 1px #85b1de; height: 20px;">
                    <td style="vertical-align: middle; text-align: center;">' . $res3['time'] . '</td>
                    <td style="vertical-align: middle; text-align: center;"><button class="download" str="' . $src1 . '">მოსმენა</button></td>
                    <td style="vertical-align: middle; text-align: center;">გამავალი</td>
                </tr>';
	}
