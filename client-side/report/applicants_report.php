...
var i 		= 0;
var done 	= ['','','','','','',''];
var aJaxURL	= "server-side/report/applicants_report.action.php";		//server side folder url
var tName   = "report";
...
function drawFirstLevel(){
		...
		var start	= $("#search_start").val();
		var end		= $("#search_end").val();
		var d_url   ="&start="+start+"&end="+end+"&done="+i+"&departament="+done[0]+"&type="+done[1]+"&category="+done[2]+"&sub_category="+done[3]+'&app='+$('#dropdown').val();
		...
		GetDataTable(tName, aJaxURL, "get_list", 4, d_url, 0, "",'','',[2,3], "<'dataTable_buttons'T><'F'Cfipl>");
		 $("#report tbody").on("click", "tr", function () {
			 if(i==2){
				 d_url1   ="&start="+start+"&end="+end+"&done="+i+"&type="+done[0]+"&category="+done[1]+"&sub_category="+done[2]+'&app='+$('#dropdown').val();
			 var nTds = $("td", this);
		            var rID = $(nTds[1]).text();
